...
def main():
    ...
    train_df = load_data('/Training_set/Formatted_Training_Data.csv')
    ...
    if Config.EXPLORATION_PLOTS:
        ...
        pairplot = sns.pairplot(train_df, diag_kind='kde', plot_kws={'alpha': 0.6, 's': 80, 'edgecolor': 'k'},
                                height=2.5)
        ...
        plt.tight_layout()
        plt.savefig('data_distribution.png', dpi=300, bbox_inches='tight')  # Save with higher DPI
        plt.close()
        ...
        if Config.EXPLORATION_PLOTS:
            ...
            # Adjust font sizes and title
            plt.title('Feature Correlation Matrix', fontsize=20, pad=20)
            plt.xticks(fontsize=14, rotation=45, ha='right')
            plt.yticks(fontsize=14, rotation=0)

            plt.tight_layout()
            plt.savefig('feature_correlation.png', dpi=300, bbox_inches='tight')  # Save with higher DPI
            plt.close()

    # Data Preparation
    X = train_df[["Frequency (GHz)", "W1 (mm)", "L1 (mm)",
                  "D1 (mm)", "W2 (mm)", "L2 (mm)"]].values.astype('float32')
    ...
    with mlflow.start_run():
        ...
        callbacks = [
            ...
            ModelCheckpoint('/home/kayode-olalere/PycharmProjects/Project ANN/Codebase/Training_set/VER_3/model_advanced.h5', save_best_only=True),
        ...
        # Accuracy Percentage (R² converted to percentage)
        ...
        shap.summary_plot(shap_values, X_test_scaled[:100],
                          feature_names=["Frequency (GHz)", "W1 (mm)",
                                         "L1 (mm)", "D1 (mm)", "W2 (mm)", "L2 (mm)"])
        plt.savefig('feature_importance.png')
        plt.close()

        # Generate Predictions for New Data
        X_gen = generated_df[["Frequency (GHz)", "W1 (mm)", "L1 (mm)",
                              "D1 (mm)", "W2 (mm)", "L2 (mm)"]].values.astype('float32')
